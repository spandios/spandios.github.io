...
하지만 팩토리 메서드 패턴은 구체적인 객체 생성 과정을 다른 서브(구체)클래스로 위임하는 것이 목적이지만, `추상 팩토리 패턴`은 연관성이 있는 객체 군이 여러개 있을 경우 이들을 묶어 추상화하며, 팩토리 객체에서 집합으로 묶은 객체를 구현화 하는 생성 패턴이다

팩토리 메서드 패턴에선 하나의 팩토리가 하나의 객체를 생성 하는 반면 `추상 팩토리 패턴`은 하나의 팩토리가 여러 객체를 생성한다.
...
삼성 브랜드는 CPU는 Intel, Memory로 Samsung을 사용하하고, LG 브랜드는 CPU는 AMD, Memory는 Hynix를 사용한다고 가정해보자.
...
저번 팩토리 메서드 패턴에서 보았던 선박을 다시 살펴보자. 

### 패턴 적용 전 

선박에 Anchor와 Wheel이라는 부품이 추가되었다고 가정해보자.
...
반면 화물선은 기본적인 부품에서 더 나아가 고급 부품을 사용하게 된다면 고급 부품을 생성하는 추상 팩토리 클래스를 만들고 이를 주입하기만 하면 된다. 
